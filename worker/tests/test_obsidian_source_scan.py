...
def _make_source(tmp_path: Path, **overrides: Any) -> ObsidianSource:
    ...
    Note: cursor storage is now in the queue (``"obsidian"`` cursor key),
    not on disk.  Tests that need to seed a cursor before scanning use
    ``q.save_cursor("obsidian", serialize_file_cursor(...))``.
    """
    cfg: dict[str, Any] = {
...
async def test_multiple_vaults_independent_cursors(tmp_path: Path) -> None:
    ...
    # Build cursor manually to simulate a previous run, then seed it
    # into the queue (where ObsidianSource now reads its cursor from).
    ...
    # Scan with seeded cursor — should only see vault_b modification
    s = _make_source(tmp_path)
    q = FakeQueue()
    q.save_cursor("obsidian", serialize_file_cursor(cursor))
    events = await _run_source_briefly(s, q)
...
async def test_vault_removed_emits_deleted_events(tmp_path: Path) -> None:
    ...
    # Build cursor manually to simulate previous run, then seed the queue
    cursor: dict[str, FileEntry] = {}
    for vault in [vault_a, vault_b]:
        for fpath in vault.rglob("*"):
            if fpath.is_file() and ".obsidian" not in fpath.parts:
    ...
    # Scan with seeded cursor — vault_b files should be 'deleted'
    s = _make_source(tmp_path)
    q = FakeQueue()
    q.save_cursor("obsidian", serialize_file_cursor(cursor))
    events = await _run_source_briefly(s, q)
...
@pytest.mark.asyncio
async def test_obsidian_cursor_includes_all_vaults(tmp_path: Path) -> None:
    """The persisted cursor contains entries from every scanned vault.

    Replaces the older periodic-checkpoint test — checkpoints no longer
    exist as a separate concept; ObsidianSource writes a single merged
    cursor via ``queue.save_cursor("obsidian", ...)`` after the scan.
    """
    vaults_dir = tmp_path / "vaults"
    vaults_dir.mkdir()

    _make_vault(vaults_dir, "vault_a", {"a.md": "hello"})
    _make_vault(vaults_dir, "vault_b", {"b.md": "world"})

    s = _make_source(tmp_path)
    q = FakeQueue()
    await _run_source_briefly(s, q)

    raw = q.load_cursor("obsidian")
    assert raw is not None
    cursor = deserialize_file_cursor(raw)
...
@pytest.mark.asyncio
async def test_obsidian_cursor_persists_after_cancellation(tmp_path: Path) -> None:
    """Cancelling ObsidianSource leaves the post-scan cursor intact.

    The merged scan cursor is written before the watchdog phase begins,
    so even an immediate cancellation preserves it in the queue.
    """
    vaults_dir = tmp_path / "vaults"
    vaults_dir.mkdir()
    _make_vault(vaults_dir, "vault", {"note.md": "content"})
    ...
    raw = q.load_cursor("obsidian")
    assert raw is not None
    assert len(deserialize_file_cursor(raw)) >= 1
